...
<div class="shell">
  <header class="topbar">
    <div class="brand">
      ...
      <div>ResumeFit Pro</div>
    </div>
    <div class="top-actions">ATS analysis | Word resume output | Recruiter-ready formatting</div>
  </header>

  <main class="workspace">
    <section class="panel intake">
      <h1>Resume Optimization Workspace</h1>
      <p>Upload a DOCX resume, paste a target job description, and generate a polished ATS-aligned Word resume.</p>
      <form id="form">
        <div>
          <label for="api_key">Gemini API key</label>
          <input id="api_key" name="api_key" type="password" autocomplete="off" placeholder="Optional if configured on server">
        </div>
        <div>
          <label for="resume_file">Existing resume</label>
          <input id="resume_file" name="resume_file" type="file" accept=".docx" required>
        </div>
        <div>
        ...
        <div class="button-row">
          <button class="primary" type="submit">Analyze and optimize</button>
    ...
    <section class="panel results">
      <div class="results-head">
        <div>
          ...
          <p id="result-subtitle">Run an analysis to view score, changes, preview, and Word download.</p>
        </div>
      </div>
      <div class="score-strip">
        ...
        <div class="metric">
          ...
          <strong id="target-score" class="ready">--</strong>
        </div>
        <div class="metric">
          <span>Status</span>
          <strong id="result-state">Ready</strong>
        </div>
      </div>
      <div class="result-body">
        <div class="change-list">
          <h3>Recommended Changes</h3>
          <div id="changes" class="placeholder">No analysis yet.</div>
        </div>
        <aside class="download-box">
          <h3>Outputs</h3>
          <div id="downloads" class="placeholder">Downloads appear after optimization.</div>
        </aside>
      </div>
    </section>
  </main>
...
form.addEventListener("submit", async (event) => {
  ...
  statusBox.textContent = "Analyzing and optimizing. This can take up to 60 seconds...";
  ...
  try {
    response = await fetch("/optimize", { method: "POST", body });
    data = await response.json();
  } catch (error) {
    statusBox.className = "";
    statusBox.textContent = "Request failed. Please restart the server and try again.";
    return;
  }
  ...
  currentScore.textContent = `${data.current_ats_score}%`;
  targetScore.textContent = `${data.target_ats_score}%+`;
  resultState.textContent = "Complete";
  resultSubtitle.textContent = "Review the change summary, preview the resume, or download the Word document.";
  statusBox.textContent = "Optimization complete.";

  const changes = data.changes_to_make || [];
  changesBox.className = "";
  changesBox.innerHTML = changes.length
    ? `<ul>${changes.map((item) => `<li>${item}</li>`).join("")}</ul>`
    : "No additional changes returned.";

  downloadsBox.className = "";
  downloadsBox.innerHTML = `
    <a href="${data.analysis_url}">View analysis</a>
    <a href="${data.preview_url}">Preview resume</a>
    <a href="${data.download_docx}">Download Word document</a>
  `;
});
</script>
</body>
